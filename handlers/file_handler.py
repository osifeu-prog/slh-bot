...
def register(bot):
    ...
    def handle_document(message):
        if not is_owner(str(message.from_user.id)):
            bot.reply_to(message, "⛔ File uploads are owner-only.")
            return
        try:
            ...
            if JOURNAL_FILE.exists():
                try:
                    ...
                    journal = []
            journal.append({"ts": time.strftime("%Y-%m-%dT%H:%M:%S"), "from": str(message.from_user.id), "file_name": fname, "path": str(save_path)})
