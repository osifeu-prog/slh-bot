...
def register(bot, context=None):
    ...
    def agent_status_cmd(m):
        ...
        if not reg:
            ...
            return
        lines = [f"{aid} — {a.get('name', 'unknown')} [{a.get('status', 'unknown')}]" for aid, a in reg.items()]
    ...
    def agent_vote_cmd(m):
        ...
        gov["individual_agent_votes"][vote_key] = {"agent_id": aid, "voter": uid, "action": action, "weight": weight, "timestamp": datetime.now(timezone.utc).isoformat()}
        _save_gov(gov)
        bot.reply_to(m, f"הצבעה נרשמה:\nסוכן {aid} → {action}\nמשקל: {weight}")
    ...
    def gov_propose_cmd(m):
        ...
        proposals.append({"id": proposal_id, "type": "proposal", "title": title, "description": desc, "status": "open", "created_by": str(m.from_user.id), "created_at": datetime.now(timezone.utc).isoformat(), "votes": {"yes": 0, "no": 0, "abstain": 0, "weighted_yes": 0, "weighted_no": 0}})
        _save_gov(gov)
        bot.reply_to(m, f"הצעה #{proposal_id} נוצרה:\n📌 {title}")
    ...
    def gov_vote_cmd(m):
        ...
        if vote_key in individual_votes:
            ...
            return
        individual_votes[vote_key] = {"proposal_id": pid, "voter": uid, "choice": choice, "weight": weight, "timestamp": datetime.now(timezone.utc).isoformat()}
    ...
    def gov_tally_cmd(m):
        ...
        if ratio >= threshold:
            ...
            bot.reply_to(m, f"✅ הצעה #{pid} אושרה\nכן: {weighted_yes}\nלא: {weighted_no}\nיחס: {ratio:.2f}" + mission_msg)
        else:
            proposal["status"] = "rejected"
            _save_gov(gov)
            bot.reply_to(m, f"❌ הצעה #{pid} נדחתה\nכן: {weighted_yes}\nלא: {weighted_no}\nיחס: {ratio:.2f}")
    ...
    def session_new_cmd(m):
        ...
        if len(parts) < 3:
            ...
            return
        agent_id, summary = parts[1].strip(), parts[2].strip()
        gov = _load_gov()
        if agent_id not in gov.get("agents_registry", {}):
            bot.reply_to(m, f"סוכן {agent_id} לא נמצא.")
            return
        session_id = len(gov.get("sessions", [])) + 1
        gov.setdefault("sessions", []).append({"id": session_id, "agent_id": agent_id, "summary": summary, "status": "active", "created_at": datetime.now(timezone.utc).isoformat()})
        _save_gov(gov)
        bot.reply_to(m, f"Session #{session_id} נפתח עבור סוכן {agent_id}.\n📝 {summary}")
    ...
    def session_close_cmd(m):
        ...
        _save_gov(gov)
        bot.reply_to(m, f"Session #{session_id} נסגר.\nתוצאה: {outcome}")
