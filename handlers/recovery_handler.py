...
def register(bot, context=None):
    ...
    def recovery(m):
        if not is_owner(str(m.from_user.id)):
            bot.reply_to(m, "⛔ Recovery diagnostics are owner-only.")
            return
        try:
            lines = []
            ...
            lines.append("")
            for f in ["state/db.json", "state/agents.json", "state/ai_health.json"]:
                lines.append(f"{f}: " + ("OK" if os.path.exists(f) else "MISSING"))
            try:
                with open("state/db.json", encoding="utf-8") as x:
                    d = json.load(x)
                agents = d.get("agents", {})
                active = sum(1 for a in agents.values() if a.get("state") == "active")
                lines.append("")
