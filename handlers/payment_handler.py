...
def register_payment_handlers(bot):
    ...
    def pay_command(m):
        ...
        if not has_permission(uid, "economy.mutate_self"):
            bot.send_message(m.chat.id, "⛔ Payments are not available for this role.")
            return
    ...
    def payment_callback(call):
        if not has_permission(str(call.from_user.id), "economy.mutate_self"):
            bot.answer_callback_query(call.id, "Payments are not available for this role.")
            return
        ...
        try:
            bot.send_invoice(chat_id=call.message.chat.id, title="SLH Credits", description=f"Add {credits} credits to your SLH account", invoice_payload=f"credits_{credits}_{call.from_user.id}", provider_token=PROVIDER_TOKEN, currency="XTR", prices=prices, start_parameter=f"credits_{credits}", need_name=False, need_phone_number=False, need_email=False, is_flexible=False)
    ...
    def pre_checkout(query: PreCheckoutQuery):
        ...
        if not has_permission(str(query.from_user.id), "economy.mutate_self"):
            bot.answer_pre_checkout_query(query.id, ok=False, error_message="Payments are not available for this role.")
            return
    ...
    def successful_payment(m):
        ...
        try:
            ...
            result = economy_service.record_stars_payment(uid=uid, credits=credits, stars_paid=payment.total_amount, currency=payment.currency, telegram_payment_charge_id=payment.telegram_payment_charge_id, provider_payment_charge_id=payment.provider_payment_charge_id, referrer_uid=referrer_uid, meta={"source": "telegram_successful_payment", "invoice_payload": payload})
            ...
            print(f"[PAY] {credits} credits added to {uid}, charge_id={result['charge_id']}")
        except Exception as e:
    ...
    def fakepay(m):
        ...
        try:
            from core import economy_service
            balance = economy_service.record_transaction(uid=uid, amount=100, reason="admin:test_payment", meta={"source": "fakepay"})
