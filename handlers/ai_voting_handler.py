    ...
    if isinstance(votes, list):
        ...
        for item in votes:
            ...
            normalized[pid] = {
                ...
                "voters": item.get("voters", {}) if isinstance(item.get("voters", {}), dict) else {},
            }
        db["votes"] = normalized
    elif votes is None:
        db["votes"] = {}
...
def register(bot):
    ...
    def register_ai(m):
        ...
        for name, desc in ai_agents:
            if name not in [a.get("name") for a in agents.values()]:
                numeric_ids = [int(k) for k in agents.keys() if str(k).isdigit()]
        ...
        db["agents"] = agents
        save_db(db)
        bot.reply_to(m, f"AI agents created: {', '.join(created)}" if created else "All AI agents already exist")
    ...
    def propose(m):
        ...
        if not isinstance(votes, dict):
            votes = {}
            db["votes"] = votes
        numeric_ids = [int(k) for k in votes.keys() if str(k).isdigit()]
