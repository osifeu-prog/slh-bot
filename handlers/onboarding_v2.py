...
def register(bot, context=None):
    ...
    def onboard_start(call):
        ...
        try:
            ...
            is_owner = int(user_id) == int(OWNER_TELEGRAM_ID)
            if not can_start_onboarding(is_owner=is_owner, is_existing_user=existing_user is not None):
            ...
            bot.send_message(call.message.chat.id, "✅ הפרופיל שלך הופעל!\nהסוכן האישי שלך מוכן.\n\nברוך הבא ל-SLH OS.")
    ...
    def continue_course(call):
        ...
        bot.send_message(call.message.chat.id, "📚 הקורס הפעיל שלך: bitcoin_mastery\nשלח /lesson bitcoin_mastery 1 כדי להתחיל.")
    ...
    def system_status(call):
        ...
        bot.send_message(call.message.chat.id, "🖥 SLH OS\nשירות פעיל, DB פעיל, LLM תקין.\nשלח /doctor לדוח מלא.")
