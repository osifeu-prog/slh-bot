...
check("Audit records action", found)
...
check(
    ...
    "test_task_1" in bus.state.get("tasks", [])
)

# Emit another task and process
bus.emit("task_create", {"chat": 123, "task": "test_task_2"})
check(
    "Second task processed",
    len(bus.state.get("tasks", [])) == 2
)

# Test task_list (just check no crash)
try:
    bus.emit("task_list", {"chat": 123})
    check("Task list handler works", True)
except Exception as e:
    check("Task list handler works", False)
