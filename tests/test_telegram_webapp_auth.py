...
class TelegramWebAppAuthTests(unittest.TestCase):
    ...
    def test_future_init_data_rejected(self):
        now = int(time.time())
        with self.assertRaisesRegex(ValueError, "TELEGRAM_INIT_DATA_AUTH_DATE_INVALID"):
            validate_init_data(self.make_init_data(auth_date=now + 61), now=now)

    @patch.dict(os.environ, {"BOT_TOKEN": TOKEN}, clear=False)
    def test_duplicate_key_rejected(self):
        data = self.make_init_data(user_id=777)
        data = data.replace("&hash=", "&auth_date=1&hash=", 1)
        with self.assertRaisesRegex(ValueError, "TELEGRAM_INIT_DATA_DUPLICATE_KEY"):
