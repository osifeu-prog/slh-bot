...
def init(bot, is_admin):
    ...
    def stake_join(m):
        ...
        parts = m.text.split()
        if len(parts) < 3:
            bot.reply_to(m, "שימוש: /stake_join 500 USDT")
            return
        try:
        ...
        bot.reply_to(
            ...
            f"תשואה חודשית: ${monthly}\n"
            f"שלח /stake_confirm TXHASH לאחר ההעברה"
        )
        for admin_id in db.get("admins", []):
            try:
                bot.send_message(
                    admin_id,
                    f"משקיע חדש: {m.from_user.first_name} - ${amount} {currency}"
                )
            except Exception:
                pass
    ...
    def staking_report(m):
        ...
        active = [
            (uid, d) for uid, d in students.items()
            if d.get("staking", {}).get("active")
        ]
        total = sum(d["staking"]["amount_usd"] for _, d in active)
        monthly_out = round(
            sum(d["staking"]["amount_usd"] * d["staking"]["monthly_rate"]
                for _, d in active), 2
        )
        msg = (
            f"דוח סטייקינג\n"
            f"משקיעים: {len(active)}\n"
            f"סה\"כ מנוהל: ${total}\n"
            f"תשלום חודשי: ${monthly_out}\n\n"
        )
        for uid, d in active:
            s = d["staking"]
