...
def stake_locked(uid, amount, lock_days=30, meta=None):
    ...
    def mutate(db):
        ...
        db.setdefault("ledger", []).append({
            ...
            "meta": {**meta, "position_id": position_id, "lock_days": lock_days, "staked_before": staked, "staked_after": staked + amount},
        })

        return {"position": positions[position_id], "credits": credits - amount, "staked": staked + amount}
...
def unstake_locked(uid, position_id, meta=None):
    ...
    def mutate(db):
        ...
        db.setdefault("ledger", []).append({
            ...
            "meta": {**meta, "position_id": position_id, "staked_before": staked, "staked_after": staked - amount},
        })

        return {"status": "unlocked", "position": position, "credits": credits + amount, "staked": staked - amount}
